...
export const buildInboundWebhookExecutionLogFilter = (
  connectorId: string,
  connectorTypeId: string
): string =>
  [
    `kibana.action.id: ${JSON.stringify(connectorId)}`,
    `kibana.action.type_id: ${JSON.stringify(connectorTypeId)}`,
  ].join(' and ');
  ...
  http.post<AsApiContract<IExecutionLogResult>>(
    `${INTERNAL_BASE_ACTION_API_PATH}/_global_connector_execution_logs`,
    {
      body: JSON.stringify({
        ...
        filter: buildInboundWebhookExecutionLogFilter(connectorId, connectorTypeId),
