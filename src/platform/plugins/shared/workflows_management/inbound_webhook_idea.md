...
Management resolves the UUID to an encrypted, space-scoped mapping containing
the connector ID and the API key granted for the latest editor who successfully
saved the webhook.
...
The UUID-to-connector mapping must not be stored in a file on the Kibana node.
...
interface InboundWebhookConfig {
   ...
   * This is safe to persist outside encrypted secrets because the UUID has
   * sufficient entropy and cannot feasibly be recovered from its SHA-256 hash.
   */
  webhookKeyHash: string;
  /**
   * Non-secret UUID regenerated by the client for every create/update request.
   * Used to correlate the persisted connector revision with ESO promotion.
   */
...
interface InboundWebhookSecrets {
  /**
   * Full externally callable URL, including the UUID bearer credential.
   */
  webhookUrl: string;
}
...
1. UI generates a UUID and sends:
   - `secrets.webhookUrl`
   - `config.webhookKeyHash`
   - a new `config.credentialRevision`
2. `preSaveHook` parses the UUID from `webhookUrl`, recomputes SHA-256, and
   rejects the save if it does not match `config.webhookKeyHash`.
...
1. Generate a new UUID, URL, and hash.
...
1. Validates `webhookKey` as a UUID.
...
The UUID is the webhook's bearer credential. Anyone with the URL can invoke the
connector.
...
UUIDv4 provides approximately 122 bits of randomness, which is sufficient for a
bearer secret if generated with `crypto.randomUUID()`. A 256-bit random token
would be stronger, but UUIDv4 is retained here as an explicit product
requirement.
...
Never include the webhook UUID, full URL, request `Authorization`/`Cookie`
...
1. Generate the key with browser `crypto.randomUUID()`.
...
generate a new UUID, grant new credentials, and create a destination-scoped
mapping. Moving a connector is implemented as destination creation followed by
source deletion only after destination activation succeeds.
...
User opens connector form
  -> UI generates UUID, URL, and SHA-256 hash
...
- valid UUID decrypts credentials and executes through a scoped Actions client;
- invalid UUID, missing mapping, inactive mapping, wrong space, stale connector,
  and wrong connector type all return the same `404`;
