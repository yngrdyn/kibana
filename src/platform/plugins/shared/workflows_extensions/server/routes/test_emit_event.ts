...
/**
 * Test route for event ingestion.
 * Allows manual triggering of events for testing purposes.
 */
export function registerTestEmitEventRoute(
  router: IRouter,
  getStartServices: () => Promise<[any, any, unknown]>
) {
  router.post(
    {
      path: '/internal/workflows_extensions/test/emit_event',
      validate: {
        body: schema.object({
    ...
    async (context, request, response) => {
      try {
        const [, , workflowsExtensions] = await getStartServices();
        ...
        const result = await (workflowsExtensions as WorkflowsExtensionsServerPluginStart).emitEvent({
