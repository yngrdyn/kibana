...
Each persisted document includes **`object.sequence`**: a monotonic revision assigned by `ChangeHistoryClient` per `(spaceId, module, dataset, objectType, objectId)`. Callers do not provide this field on `ObjectChange`; use `object.sequence` as the primary ordering key for history.
...
- Returns change documents for the given object `type` and `id` in the specified Kibana space, sorted by `object.sequence`, then `@timestamp`, then `event.id` as a tie-breaker. Supports pagination and custom sort/filters via `opts`.

All persisted documents follow the same schema (see below).

### Event IDs

Each persisted document gets a unique `event.id` assigned by the package (UUID v7). `getHistory()` uses `event.id` as a final tie-breaker after `object.sequence` and `@timestamp`.
...
- **`getHistory(spaceId, objectType, objectId, opts?)`**
  ...
  - Results are sorted by `object.sequence`, then `@timestamp`, and `event.id` as the tie-breaker.
...
| `object.sequence`       | `integer`   | Autogenerated version identifier.   |
