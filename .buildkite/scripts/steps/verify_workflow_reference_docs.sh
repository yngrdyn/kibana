...
# Boots Elasticsearch + Kibana, waits for extension step registrations, runs the generators,
# and fails if git detects changes to the snippet files.
...
report_step "Wait for server extension registrations"
node scripts/wait_for_workflow_extension_registrations.js
...
if [ "$diff_status" -ne 0 ]; then
  ...
  echo ""
  echo "The following files differ from what the registries produce:"
  for f in "${STEP_DOCS_FILES[@]}" "${TRIGGER_DOCS_FILES[@]}" "$WORKFLOW_REFERENCE_TOC_FILE"; do
    echo "  - $f"
  done
  echo ""
  echo "To fix locally:"
  echo "  1. Start Elasticsearch and Kibana."
  echo "  2. Run: node scripts/wait_for_workflow_extension_registrations.js"
  echo "  3. Run:"
  echo "       node scripts/generate workflow-step-docs"
  echo "       node scripts/generate workflow-trigger-docs"
  echo "  4. Commit the updated snippet files and docs/reference/toc.yml."
  echo ""
